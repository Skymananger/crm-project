...
const mapDBToLead = (l: any): Lead => {
  ...
  // Critério Estrito do Usuário (Prioridade Máxima para Notas):
  if (notes.includes('importado million') || notes.includes('million')) {
    ...
    inferredType = 'dign';
  } else if (inferredType === 'new' && (stage.includes('SKY') || stage === 'PRODUTOS' || interest.includes('royal') || interest.includes('prestige'))) {
    inferredType = 'sky';
  }

  // Se o lead tem produtos registrados nas notas, ele é um cliente SKY
    ...
    try {
      ...
      setLeads(prev => {
        ...
        for (const line of lines) {
          ...
          const [nome, cidade, fone, celular] = parts;
          ...
          if (leadsMap.has(whatsapp)) {
            ...
            if (existing.pipelineType !== 'million') {
              leadsMap.set(whatsapp, { 
                ...
                prospectNotes: 'Importado Million\n' + (existing.prospectNotes || '')
              });
              updatedCount++;
            }
          } else {
    ...
    try {
      ...
      setLeads(prev => {
        ...
        for (const line of lines) {
          ...
          const [nome, cidade, telRes, celular] = parts;
          if (!nome || nome.trim() === '') continue;
        ...
        for (const line of lines) {
          ...
          if (leadsMap.has(whatsapp)) {
            ...
            if (!isMillion && !isDign && existing.pipelineType !== 'sky') {
              ...
              leadsMap.set(whatsapp, { ...existing, pipelineType: 'sky', stage: CustomerPipelineStage.CLIENTES_SKY });
              updatedCount++;
            }
          } else {
            // Cria novo lead
    ...
    try {
      ...
      setLeads(prev => {
        ...
        for (const line of lines) {
          ...
          // Se não encontrou telefone válido, gera um ID único para não sobrescrever
          const uniqueKey = cleanPhone && cleanPhone.length >= 8 ? cleanPhone : `sem-telefone-${Date.now()}-${Math.random()}`;
