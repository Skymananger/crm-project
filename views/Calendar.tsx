      ...
      <main className="flex-1 flex flex-col min-w-0 bg-white">
        ...
        <header className="h-16 border-b border-slate-200 flex items-center justify-between px-4 shrink-0">
          <div className="flex items-center gap-2 lg:gap-4 overflow-x-auto no-scrollbar py-2">
            <h1 className="text-sm lg:text-xl text-[#003459] font-black uppercase tracking-widest truncate max-w-[120px] lg:max-w-none">
              {currentDate.toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' })}
            </h1>
            <div className="flex items-center gap-1 shrink-0">
              <button onClick={() => setCurrentDate(new Date())} className="px-4 py-2 bg-blue-50 text-blue-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-100 transition-all active:scale-95 shadow-sm">Hoje</button>
              <button onClick={() => navigate('prev')} className="p-2 hover:bg-slate-100 rounded-xl text-slate-600 transition-all active:scale-90"><ChevronLeft size={20}/></button>
              <button onClick={() => navigate('next')} className="p-2 hover:bg-slate-100 rounded-xl text-slate-600 transition-all active:scale-90"><ChevronRight size={20}/></button>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {!googleTokens && (
              <button 
                ...
                className="flex items-center justify-center p-2.5 hover:bg-slate-100 rounded-2xl text-slate-600 border border-slate-200 transition-all active:scale-95"
                title="Conectar Google Agenda"
              >
                <RefreshCw size={20} />
                <span className="text-[10px] font-black uppercase tracking-tighter ml-2 hidden md:inline">Sincronizar</span>
              </button>
            )}
            <div className="flex bg-slate-100/50 p-1 rounded-2xl border border-slate-200">
              {(['day', 'week', 'month'] as CalendarViewType[]).map(v => (
                <button 
                  ...
                  className={`px-4 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${view === v ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                >
